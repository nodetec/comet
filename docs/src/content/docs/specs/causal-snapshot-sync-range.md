...
- each `vc` tag must include a non-empty opaque `device_id`
- each `vc` counter must be a non-negative integer
...
- Define bootstrap, replay, and live-follow behavior for causal snapshot sync events
- Define additional encrypted payload semantics for concrete profiles
- Define additional kinds inside the sync range if future causal snapshot sync families need them
