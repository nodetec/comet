...
class Accumulator {
  constructor() {
    ...
    if (typeof window === "undefined") {
      ...
      // browser
      this.sha256 = async (slice) =>
        new Uint8Array(await crypto.subtle.digest("SHA-256", slice));
    }
  }
...
class Negentropy {
  ...
  async initiate() {
    ...
    await this.splitRange(
      0,
      this.storage.size(),
      this._bound(Number.MAX_VALUE),
      output,
    );
  ...
  async reconcile(query) {
    ...
    while (query.length !== 0) {
        ...
        if (this.isInitiator) {
          ...
          this.storage.iterate(lower, upper, (item) => {
            ...
            if (!theirElems[k]) {
              ...
              if (this.isInitiator)
                haveIds.push(
                  this.wantUint8ArrayOutput ? k : uint8ArrayToHex(k),
                );
            } else {
      ...
      if (this.exceededFrameSizeLimit(fullOutput.length + o.length)) {
        ...
        let remainingFingerprint = await this.storage.fingerprint(
          upper,
          storageSize,
        );
  ...
  async splitRange(lower, upper, upperBound, o) {
    ...
    if (numElems < buckets * 2) {
      ...
      for (let i = 0; i < buckets; i++) {
        ...
        let ourFingerprint = await this.storage.fingerprint(
          curr,
          curr + bucketSize,
        );
        curr += bucketSize;
  ...
  getMinimalBound(prev, curr) {
    if (curr.timestamp !== prev.timestamp) {
      ...
      return this._bound(
        curr.timestamp,
        curr.id.subarray(0, sharedPrefixBytes + 1),
      );
    }
  }
}
...
const uint8ArrayToHexLookupTable = new Array(256);
{
  ...
  for (let i = 0; i < 256; i++) {
    uint8ArrayToHexLookupTable[i] =
      hexAlphabet[(i >>> 4) & 0xf] + hexAlphabet[i & 0xf];
  }
}
