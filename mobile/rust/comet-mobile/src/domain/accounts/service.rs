...
pub fn list_accounts(state: &Arc<AppState>) -> Result<Vec<AccountSummary>, AppError> {
    let conn = app_database_connection(state)?;
...
pub fn add_account(state: &Arc<AppState>, nsec: &str) -> Result<AccountSummary, AppError> {
    let staged_dir = staged_account_dir(state)?;
    ...
    let creation_result = (|| -> Result<AccountSummary, AppError> {
        ...
        let target_dir = account_dir_for_npub(state, &account.npub)?;
        ...
        crate::adapters::mobile::key_store::store_account_nsec(
            state,
            &account.public_key,
            &identity.nsec,
        )?;
    ...
    if creation_result.is_err() {
        ...
        if let Some(public_key) = stored_key_public_key.as_deref() {
            crate::adapters::mobile::key_store::remove_account_nsec(state, public_key);
        }
    }
...
pub fn switch_account(state: &Arc<AppState>, public_key: &str) -> Result<AccountSummary, AppError> {
    let mut conn = app_database_connection(state)?;
    let account = load_account_record_by_public_key(state, &conn, public_key)?
...
fn staged_account_dir(state: &Arc<AppState>) -> Result<PathBuf, AppError> {
    Ok(accounts_root_dir(state)?.join(format!("{STAGED_ACCOUNT_PREFIX}{}", now_millis())))
}
