...
pub async fn publish_note(
    ...
    let content = if let Some(ref blossom_url) = blossom_url {
        crate::adapters::blossom::client::upload_and_rewrite_attachments(state, blossom_url, &content, &keys).await?
    } else {
        content
    };
    ...
    let relay_count = relay_urls.len() as u64;
    ...
    let success_count = result.success.len() as u64;
    let fail_count = result.failed.len() as u64;
